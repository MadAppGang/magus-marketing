...
![Flow diagram: a question is sorted into a mode, routed to one of two investigation skills, run by the read-only analyze agent over search, file reading and whichever structural tools the configured engine supports, and returned as findings with file and line evidence](./images/dev-investigate-flow.svg)
...
## If `code-search` isn't installed
...
| You want | Guide |
|---|---|
| The same thing, without going through `dev` | [code-search](./code-search.md) |
