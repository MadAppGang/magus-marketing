...
- **BREAKING — `seo`, `instantly` and `claudish` no longer ship on `magus-marketing`.**
  `plugins/seo/` and `plugins/instantly/` are deleted. `claudish` stays on `magus`. The
  channel ships `image-generate` and `video-editing`.

### Migration notes

Remove `seo@magus-marketing` and `instantly@magus-marketing` from `enabledPlugins`; they
have no replacement. Replace `claudish@magus-marketing` with `claudish@magus`.
...
- Channel version aligned with Marketplace 10.3.0: carries `claudish` v2.0.1, the runtime
  dependency dual-published here. No other marketing plugin changed.
...
- Channel version aligned with Marketplace 9.3.1: carries `claudish` v1.0.1, the
  runtime dependency dual-published here. No other marketing plugin changed.
...
- Channel version aligned with the Marketplace 9.3.0 dispatch-integrity release it
  carries: `video-editing` v1.2.1. No content beyond
...
- First release of the `magus-marketing` marketplace
  ...
  | `claudish` | 1.0.0 | MCP runtime; dual-published |
